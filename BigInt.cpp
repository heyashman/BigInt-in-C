...
public:
	...
	//Helper Functions:
	friend void half(BigInt &a);
	friend bool isZero(const BigInt &);
	friend int numDigits(const BigInt &);
	...
	friend BigInt sqrtBigInt(BigInt &a);
	...
	//Others
	friend BigInt getNthCatalan(int n);
	friend BigInt getNthFibonacci(int n);
...
bool operator<(const BigInt&a,const BigInt&b){
	int n = numDigits(a), m = numDigits(b);
	if(n != m)
		return n < m;
	while(n--)
...
{
	BigInt first("12345");
	cout << "The number of digits in first big integer = " << numDigits(first) << '\n';
	BigInt second(12345);
	if (first == second)
	{
	...
	first++;
	cout << "After incrementing the value of first is : " << first << '\n';
	...
	for (int i = 0; i <= 100; i++)
	{
		BigInt fact = getFactorial(i);
		cout << "Factorial of " << i << " = " << fact << '\n';
	}
}

//Contributed By: Ashmandeep Singh Joshan
